...
public class UpdatePasswordCommandHandler(IUnitOfWork unitOfWork, IUserContext userContext, IPasswordCheck passwordCheck, IPasswordHash passwordHash) : ICommandHandler<UpdatePasswordCommand, Result>
{
    public async Task<Result> Handle(UpdatePasswordCommand request, CancellationToken cancellationToken)
    {

        var userId = UserId.Of(userContext.UserId!.Value);
        var user = await unitOfWork.Users.SingleOrDefaultAsync(new GetUserByIdGlobalSpecification(userId), cancellationToken);
        if (user == null)return UserErrors.NotFound(userContext.UserId!.Value);
